...
const SketchedButton = ({ text, width, fontsize, scale, scale_after, pointerCursor, onClickHandler }) => {
    return (
            <button
                className={`sketch_button ${scale ? 'scale' : '' } ${scale_after ? 'scale_after' : '' }`}
                style={{width: width, fontSize: fontsize, cursor: pointerCursor ? 'pointer' : 'none'}}
